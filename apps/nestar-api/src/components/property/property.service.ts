...
export class PropertyService {
  ...
  public async createProperty(input: PropertyInput): Promise<Property> {
    try {
      const result = await this.propertyModel.create(input); // schemani olib beradi
  ...
  private shapeMatchQuery(match: T, input: PropertiesInquiry): void {
    ...
    if (pricesRange) match.propertyPrice = {
      $gte: pricesRange.start,
      $lte: pricesRange.end,
    };

    if (periodsRange) match.createdAt = {
      $gte: periodsRange.start,
      $lte: periodsRange.end,
    };

    if (squaresRange) match.propertySquare = {
      $gte: squaresRange.start,
      $lte: squaresRange.end,
    };

    if (text) match.propertyTitle = {
  ...
  public async likeTargetProperty(memberId: ObjectId, likeRefId: ObjectId
  ): Promise<Property> {
    //@ts-ignore
    ...
    if (!result) throw new InternalServerErrorException(Message.SOMETHING__WENT_WRONG);
